...
fn initialize_gop(system_table: &SystemTable<Boot>) -> FramebufferInformation {
...
fn map_segment(
    ...
    if segment.mem_size() > segment.file_size() {
        ...
        if zero_start.as_u64() & 0xfff != 0 {
            ...
            unsafe { new_frame_ptr.write(ZERO_PAGE_ARRAY_SIZE_FOUR_KIB) };
...
fn create_boot_info<I>(
    used_entries: &mut LevelFourEntries,
...
fn switch_to_kernel(
    kernel_info: LightsaberSystemKernelInformation,
    ...
    unsafe {
        let kernel_level_4_start = page_tables.kernel_level_four_frame.start_address().as_u64();
...
fn efi_main(image: Handle, system_table: SystemTable<Boot>) -> Status {
    ...
    let kernel_bin = load_file(system_table.boot_services(), LIGHTSABER_SYSTEM_KERNEL_ELF_PATH);
    ...
    let mmap_buffer = unsafe { slice::from_raw_parts_mut(buffer_ptr, buffer_size) };
    ...
    let boot_info = BootInformation { framebuffer_information: frame_buffer_info };
